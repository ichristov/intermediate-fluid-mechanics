...
### tensor/tensor (direct product)

(matrix $\times$ matrix = matrix) &mdash; *does not commute*:

$$
    \underline{\underline{R}} = \underline{\underline{T}} ~ \underline{\underline{S}} \quad\text{or}\quad R_{ij} = T_{ik}S_{kj}
$$

but (for example)

$$
    T_{ik}S_{jk} = \underline{\underline{T}} ~ \underline{\underline{S}}^t
