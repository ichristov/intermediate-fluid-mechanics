...
\]
\textbf{Note:} If we always write $T_{ij}$, then you can remember what index to put on $v$ by whether $\underline{v}$ is in-front of $\underline{\underline{T}}$ (then $i$, \textcolor{red}{and $\underline{v}$ is a row vector}) or behind $\underline{\underline{T}}$ (then $j$, \textcolor{red}{and $\underline{v}$ is a column vector}) in the symbolic notation expression.
