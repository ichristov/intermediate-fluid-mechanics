...
% Patch booktabs rules (\toprule, \midrule, \bottomrule) to be PDF artifacts.
% \makeatletter wraps \AddToHook so the body is tokenized with @ as a letter —
% the hook body is tokenized at call time, not at execution time, so
% \makeatletter *inside* the body would be too late for \@BTnormal etc.
% Note: \cmidrule is already patched by latex-lab-testphase-firstaid.sty.
% Note: \pdfliteral is pdfLaTeX-specific; LuaLaTeX/XeLaTeX would need changes.
\makeatletter
...
% Patch footnote separator rule to be a PDF artifact.
% \footnoterule runs in vertical mode so \pdfliteral works fine here.
\def\footnoterule{%
  \kern-3\p@
  \pdfliteral{/Artifact BMC}%
...
\section*{A Brief Handbook of Elementary Ideal Flows\blfootnote{Inspired by Tables 12.1 and 12.2 of Granger's \emph{Fluid Mechanics} (Dover Publications, 2012). Flow \circled{1.}~--~\circled{5.} are the ``building blocks'' and the rest are superposition examples.}}
