...
\usepackage[pdftitle={iNS Summary Sheet}, pdflang=en-US]{hyperref}
...
\section*{\centering\fbox{Compressible flow}}
\begin{align}
...
\begin{equation}
    \rho\frac{D \underline{v}}{D t} = \rho\underline{F} + \nabla\cdot\underline{\underline{T}} \tag*{\text{\small[COLM v2]}}
\end{equation}
\begin{equation*}
...
\hfill
\begin{minipage}[t]{0.45\textwidth}
\section*{\centering\fbox{Incompressible flow}}
...
Don't be deceived because COLM v2 looks the same in both columns. Here $\rho_0=const.$ (we're not solving for it), on the left $\rho = \rho(x,y,z,t)$ (we're solving for it).
